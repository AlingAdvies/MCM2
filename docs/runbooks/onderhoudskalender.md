...
|---|---|---|
| **Sleutel- en wachtwoordrotatie** | `.env` bevat het Supabase-productiewachtwoord, `RESEND_API_KEY`, `TELEGRAM_BOT_TOKEN` en de OIDC-secrets. Daarnaast twee sleutels met een **harde einddatum**: `/opt/mcm2/.ghcr-token` op saxombp verloopt rond **2026-11-08** (dan stopt elke uitrol), en de `TAILSCALE_AUTHKEY` als GitHub-secret verloopt standaard na **90 dagen** — dan kan CI niet meer bij saxombp en faalt de staging-uitrol. Er is geen rotatieritme, geen vervaldatum-bewaking en geen procedure. | **Hoog** — er leeft een echte tenant, en er zijn nu twee sleutels met een einddatum |
