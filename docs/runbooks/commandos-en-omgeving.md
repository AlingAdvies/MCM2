...
| Eén specifieke route geeft 500 in productie, terwijl dezelfde route lokaal en op een verse wegwerpdatabase gewoon werkt — vaak zichtbaar als een kapotte sidebar of leeg scherm terwijl de rest van de pagina wél laadt | Een migratie die een nieuwe tabel in schema `clm` aanmaakt zonder expliciete `GRANT`, vertrouwend op `ALTER DEFAULT PRIVILEGES` uit migratie 0001. Dat werkt alleen als dezelfde rol de tabel aanmaakt als die de default-ACL zette — op productie is dat `postgres` vs. `clm_migrator` (ADR-009), dus de rechten komen er niet vanzelf. Lokaal/wegwerp loopt migratie 0001 als dezelfde rol die daarna de tabel aanmaakt, dus daar valt het nooit op. Zie ARCHITECTUUR.md Platformgarantie 3 en migraties 0022, 0039 | `SELECT grantee, privilege_type FROM information_schema.role_table_grants WHERE table_schema='clm' AND table_name='<tabel>'` tegen de betreffende omgeving; ontbreekt `clm_api`/`clm_admin`, dan een migratie met expliciete `GRANT SELECT, INSERT, UPDATE [, DELETE] ON clm.<tabel> TO clm_api, clm_admin`. `scripts/verify-omgevingen.js` (`zonderLeesrecht`) meldt dit voortaan zonder dat een gebruiker eerst tegen de 500 hoeft te lopen |
