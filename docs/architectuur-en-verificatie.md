...
**Opgesteld:** 2026-07-31 · **Bijgewerkt:** 2026-08-21 (§8: productie draait sinds 19-08 op AWS ECS, niet meer op saxombp — de uitrolketen en de risico's daarvan zijn herzien)
...
| Doorlopende bewaking van een draaiende omgeving | **Grotendeels nog niet, sinds 19-08 deels anders voor productie** | Voor acceptatie en staging (`saxombp`) ongewijzigd: geen enkel signaal wanneer ze omvallen, je zou het merken doordat iemand belt. Voor productie op AWS bestaat sinds 19-08 wel een basaal, generiek vangnet: ECS Express Mode herstart een taak automatisch als de `/health`-check faalt, en de deploy-workflow wacht op aantoonbare gezondheid vóór hij als geslaagd geldt. Dat is **geen bewaking van een reeds langer draaiende omgeving** — het vangt alleen een falende container tijdens/na een deploy, niet een applicatie die uren later stilletjes fout gaat zonder te crashen. Er is nog geen alarmering (Telegram-achtig) voor productie, zoals die voor de backup wél bestaat. CloudWatch Logs-retentie en een AWS Budget-alert staan nog open (zie het projectgeheugen `mcm2-besluit-18-08-naar-aws`). |
...
| De uitrolketen zelf | **Beproefd, herzien 2026-08-21** | Op 2026-08-10 beproefd tegen de toenmalige opzet: acceptatie, staging en productie alle drie op één machine (`saxombp`). Sinds 2026-08-19 draait productie op AWS (ECS Express Mode, Ierland) via de nieuwe workflow `productie-aws.yml` — dezelfde vier remmen (backup, staging-beproeving, migratiestand, handmatig akkoord), maar nu voor het eerst met een **volledig automatische** container-uitrol (geen Tailscale-beperking bij een AWS-API-aanroep, in tegenstelling tot saxombp). Eerste volautomatische run geslaagd 2026-08-20, na drie configuratiefouten (OIDC-sub-claim-vorm, een te lange image-tag, een listener-regel met een vast target-group-ARN die na blue/green-omwisseling naar de lege kant bleef wijzen) — alle drie gevonden én gefixt via die run zelf, niet vooraf voorzien. Login end-to-end bevestigd in een schone incognito-sessie. **Wat dit verandert aan het beschikbaarheidsrisico:** productie staat niet meer op één machine zonder reservestroom — dat risico geldt nu alleen nog voor acceptatie en staging, die bewust op `saxombp` blijven (acceptatie is wegwerp/oefenomgeving; staging beproeft primair de database-verbindingslaag, niet de hostingbeschikbaarheid). Wat **niet** verandert: rollback op AWS is dezelfde workflow opnieuw draaien met de vorige versienummers — nog niet apart beproefd sinds de eerste geslaagde run. |
| Sleutel- en wachtwoordrotatie | **Bestaat niet** | Geen ritme, geen vervaldatum, geen procedure. Raakt het Supabase-productiewachtwoord, de Resend-sleutel, het Telegram-token, de OIDC-secrets, het GHCR-token op `saxombp` (verloopt ~8 november 2026) en, sinds 19-08, de AWS Secrets Manager-secrets voor productie (`mcm2/productie/*`) en het GHCR Personal Access Token dat ECS gebruikt om images te pullen. De OIDC-authenticatie tussen GitHub Actions en AWS heeft hier bewust geen langlevende sleutel voor nodig — die loopt via een kortlevend token per run (IAM Identity Provider + rol `GitHubActions-MCM2-ECS-Deploy`), dus dát specifieke pad heeft geen rotatieprobleem. Staat als gat in `docs/runbooks/onderhoudskalender.md` §5. |
...
- `.github/workflows/productie-aws.yml` — de AWS-uitrolworkflow, sinds 2026-08-19/20
- `docs/runbooks/devops-handleiding.md` — bijgewerkt 2026-08-21 naar de AWS-situatie
