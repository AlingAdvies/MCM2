...
- **Zwaarste openstaande risico van het project (Issue #19): de backup van de Supabase-database is nooit getest.** Niet "we vermoeden dat het goed zit" — het is nooit geprobeerd. Als `clm-enterprise` omvalt, is onbekend of er iets herstelbaar is. Op 2026-07-28 verhoogd van `priority:before-production` naar `priority:before-pilot`: die classificatie stamde uit een periode waarin er nog vrijwel niets in de database stond. Runbook geschreven (`docs/runbooks/supabase-verificatie-en-restoretest.md`, stap 1); uitvoering vereist Supabase-dashboardtoegang en ligt daarmee bij de eigenaar.
...
- Branch: `main`, up to date met `origin/main`. Working tree schoon. **Geen openstaande feature branches meer.**
...
0. ~~Beoordelen en mergen van de twee openstaande branches~~ — **afgerond 2026-07-28**: beide gemerged via PR #26 en #27, branches opgeruimd.
