...
- **Issue #7 (P0-restpunt, tenantcontext-verificatie) — in uitvoering:** vereist twee gescheiden mechanismen (interne beheerder, externe leverancier via tokengebaseerde toegang). Interne-beheerderkant is **herzien op 2026-07-27**: AWS Cognito losgelaten vóór er een User Pool werd aangemaakt (geen opruimwerk nodig), nieuw besluit is **Microsoft Entra External ID** als CIAM-laag. Zie ADR-006 (herzien) voor de volledige motivatie. Het tijdelijke AWS-account `727732213368` is niet langer nodig voor identity.
- **Entra External ID PoC — gedeeltelijk werkend, geblokkeerd op een UI-bug (2026-07-27):** tenant `mcm2ciam.onmicrosoft.com` aangemaakt, federatie met `alingadvies.nl` geconfigureerd en aantoonbaar werkend tot en met succesvolle login + MFA. Sign-up-flow loopt vast op het "Add details/Surname"-scherm: de Next-knop reageert niet (Cancel wel), oorzaak nog niet bevestigd (vermoeden: browserextensie-interferentie of stille clientside-validatie). Volledige technische bevindingen en eerstvolgende diagnosestappen: `docs/architecture-review/2026-07-27/01-entra-external-id-poc-bevindingen.md`. Niet opnieuw vanaf nul beginnen — dit document bevat alle client ID's, tenant ID's en configuratiestatus.
